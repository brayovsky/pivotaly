...
const registerToken = (context, storyInfoDataProvider) => {
  ...
  return tokenPromise.then(token => {
    ...
    validate('token', context, true, 'Invalid token. Make sure you have copied the right value', [context, storyInfoDataProvider]).then(() => {
      window.showInformationMessage('Pivotal Tracker API token updated successfully')
      validate('projectID', context, true, 'Invalid ProjectID. Please pick a valid project.', [context, storyInfoDataProvider]).then(() => {
