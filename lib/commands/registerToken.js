...
const onRej = _rej => window.showWarningMessage('Token not set. You will not be able to use all pivotaly features')

const registerToken = (context, statusBarItem) => {
  ...
  return tokenPromise.then(token => {
    if(!token) return onRej(token)

    window.showInformationMessage('Validating token')

    validate('token', context, true, 'Invalid token. Make sure you have copied the right value', [context, statusBarItem]).then(_didValidationSucceed => {
      ...
      validate('projectID', context, true, 'Invalid ProjectID. Please pick a valid project.', [context, statusBarItem]).then(_didProjectValidationSucceed => {
