...
describe('#git', () => {
  ...
  describe('checkoutFromBranch', () => {
    ...
    test('should call listenForCheckout after a successful checkout', async () => {
      jest.doMock('simple-git', () => () => ({
        branchLocal: jest.fn(cb => cb(null, {
          current: new Chance().string(),
          all: ['branch-2', 'branch']
    ...
    test('should call listenForCheckout after an usuccessful checkout', async () => {
      jest.doMock('simple-git', () => () => ({
        branchLocal: jest.fn(cb => cb(null, {
          current: new Chance().string(),
          all: ['branch-2', 'branch']
    ...
    test('should reject with error if checkout was unsuccessful', async () => {
      jest.doMock('simple-git', () => () => ({
        checkoutBranch: jest.fn((newBranch, base, cb) => cb('err'))
      }))
      checkoutFromBranch = require('./git').checkoutFromBranch
