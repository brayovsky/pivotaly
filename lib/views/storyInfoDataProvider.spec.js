  ...
  describe('getChildren', () => {
    beforeEach(() => {
      PtTasks.mockImplementation(() => ({
        getAllTasks: () => ({data: 'data'})
      }))
      PtBlockers.mockImplementation(() => ({
        getBlockers: () => ({data: 'data'})
      }))
    ...
    it('should get tasks if a tree item label is Tasks', async () => {
      ...
      const ptTaskInstance = PtTasks.mock.instances
      expect(ptTaskInstance).toHaveLength(1)
    ...
    it('should get blockers if a tree item label is Blockers', async () => {
      ...
      const ptBlockersInstance = PtBlockers.mock.instances
      expect(ptBlockersInstance).toHaveLength(1)
