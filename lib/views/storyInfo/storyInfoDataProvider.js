...
class StoryInfoDataProvider extends PivotalyDataProvider {
  ...
  async getChildren(element) {
    ...
    const Tasks = new PivotalyTreeItem('Tasks', this, 'taskTitle-#', TreeItemCollapsibleState.Collapsed)
    Tasks.iconPath = {
    ...
    const Blockers = new PivotalyTreeItem('Blockers', this, 'blockerTitle-#', TreeItemCollapsibleState.Collapsed)
    Blockers.iconPath = {
  ...
  // TODO: Decouple
  // Remove this and use setter to set story then refresh externally
  refresh(story) {
    if (story) {
      this.story = story
    } else {
      // Remove this block by entirely relying on story passed
      // If no story
